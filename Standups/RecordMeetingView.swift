...
struct MeetingHeaderView: View {
  ...
  private var totalDuration: Duration {
    return .seconds(self.secondsElapsed) + self.durationRemaining
  }
...
struct MeetingTimerView: View {
  ...
  private var currentSpeakerName: String {
    guard self.speakerIndex < self.standup.attendees.count else { return "Someone" }
...
struct MeetingFooterView: View {
  ...
  private var speakerText: String {
    guard self.speakerIndex < self.standup.attendees.count - 1 else { return "No more speakers" }
    return """
